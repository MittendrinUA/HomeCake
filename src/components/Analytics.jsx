  ...
  return (
    <div className="p-4 pb-28">
      <div className="flex bg-[#1E1919] border border-[#2A2323] rounded-[20px] p-1.5 mb-6 shadow-sm overflow-x-auto custom-scrollbar">
        <button onClick={() => setPeriod('month')} className={`flex-1 min-w-[80px] py-2.5 text-[10px] uppercase tracking-widest font-bold rounded-xl transition-all ${period === 'month' ? 'bg-[#151212] text-[#D4AF37] shadow-md border border-[#2A2323]' : 'text-[#8C7A7A]'}`}>Місяць</button>
        <button onClick={() => setPeriod('year')} className={`flex-1 min-w-[80px] py-2.5 text-[10px] uppercase tracking-widest font-bold rounded-xl transition-all ${period === 'year' ? 'bg-[#151212] text-[#D4AF37] shadow-md border border-[#2A2323]' : 'text-[#8C7A7A]'}`}>{currentYear} рік</button>
        <button onClick={() => setPeriod('lastYear')} className={`flex-1 min-w-[80px] py-2.5 text-[10px] uppercase tracking-widest font-bold rounded-xl transition-all ${period === 'lastYear' ? 'bg-[#151212] text-[#D4AF37] shadow-md border border-[#2A2323]' : 'text-[#8C7A7A]'}`}>{currentYear - 1} рік</button>
        <button onClick={() => setPeriod('all')} className={`flex-1 min-w-[80px] py-2.5 text-[10px] uppercase tracking-widest font-bold rounded-xl transition-all ${period === 'all' ? 'bg-[#151212] text-[#D4AF37] shadow-md border border-[#2A2323]' : 'text-[#8C7A7A]'}`}>Весь час</button>
      </div>

      <div className="flex items-center justify-between mb-4">
        <h2 className="text-[#F4EFEA] text-xl font-bold tracking-wide">Фінанси (Видано)</h2>
        <span className="text-[#8C7A7A] text-xs font-medium">{filteredSales.length} замовлень</span>
      </div>

      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="bg-[#1E1919] border border-[#2A2323] p-5 rounded-[24px] shadow-lg">
          <p className="text-[#8C7A7A] text-[10px] uppercase tracking-widest font-bold mb-1">Виручка</p>
          <p className="text-[#F4EFEA] text-2xl font-bold">{tr.toFixed(2)} ₴</p>
        </div>
        <div className="bg-[#1E1919] border border-[#2A2323] p-5 rounded-[24px] shadow-lg">
          <p className="text-[#8C7A7A] text-[10px] uppercase tracking-widest font-bold mb-1">Собівартість</p>
      ...
      <div className="bg-gradient-to-br from-[#D4AF37]/20 to-[#151212] border border-[#D4AF37]/30 p-6 rounded-[32px] mb-8 shadow-lg relative overflow-hidden">
        ...
        <p className="text-[#D4AF37] text-xs uppercase font-bold tracking-widest mb-1">Чистий Прибуток</p>
        ...
        <div className="flex justify-between items-center mb-4">
           <p className="text-[#8C7A7A] text-xs font-medium tracking-wide">Рентабельність (маржа)</p>
        ...
        {totalWasteLoss > 0 && (
          <div className="border-t border-[#D4AF37]/20 pt-3 mt-1 flex justify-between items-center">
             <p className="text-red-400/80 text-[10px] uppercase tracking-widest font-bold">Збитки (Брак)</p>
             <p className="text-red-400 font-bold text-sm">-{totalWasteLoss.toFixed(2)} ₴</p>
          </div>
        )}
      </div>

      <h2 className="text-[#F4EFEA] text-xl font-bold mb-4 tracking-wide">Активи складу (Поточні)</h2>
      <div className="bg-[#1E1919] border border-[#2A2323] p-6 rounded-[32px] shadow-lg mb-8 flex items-center justify-between">
        <div>
           <p className="text-[#8C7A7A] text-[10px] uppercase tracking-widest font-bold mb-1">Грошей у товарі</p>
      ...
      {topC.length > 0 && (
        <>
          <h2 className="text-[#F4EFEA] text-xl font-bold mb-4 tracking-wide flex items-center gap-2"><Star size={20} className="text-[#D4AF37]"/> Топ Клієнти (за обраний час)</h2>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[#8C7A7A] text-[10px] uppercase tracking-widest">{c.count} замовлень</p>
                  </div>
                </div>
