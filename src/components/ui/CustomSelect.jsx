...
export default function CustomSelect({ value, onChange, options, label = "Виберіть опцію", className, searchable = false }) {const { t } = useTranslation();
      ...
      <AnimatePresence>
        ...
        <div className="fixed inset-0 z-[120] flex flex-col justify-end items-center">
            ...
            <div className="mb-4 shrink-0 bg-[#1E1919] border border-[#2A2323] rounded-2xl p-3 flex items-center gap-3">
                  ...
                  <input
                type="text"
                placeholder={t("auto.t_126", "Пошук...")}
                value={search}
              ...
              <p className="text-[#8C7A7A] text-center text-sm py-4">{t("auto.t_127", "Нічого не знайдено")}</p> :
