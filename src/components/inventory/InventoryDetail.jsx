...
const InventoryDetail = memo(function InventoryDetail({ item, onUpdate, onDelete, onWaste, askConfirm, askPrompt, showToast, logs, logMovement, compressImage }) {const { t } = useTranslation();
  ...
  return (
    <div className="p-4 pb-24">
      <div className="bg-[#1E1919] border border-[#2A2323] rounded-[32px] p-6 mb-6 shadow-lg">
        ...
        <div className="flex justify-between items-start mb-8 gap-2">
          <div className="flex items-center gap-4">
             ...
             <div>
               <label className="text-[#8C7A7A] text-xs uppercase font-bold tracking-widest block mb-1">{t("auto.t_31", "Товар")}</label>
               <h2 className="text-2xl font-bold text-[#F4EFEA] leading-tight pr-2">
                 {name}
                 {isMix && <span className="ml-2 text-[#2AABEE] bg-[#2AABEE]/20 border border-[#2AABEE]/30 px-2 py-1 rounded-lg text-[9px] uppercase font-bold align-middle">{t("auto.t_32", "Мікс")}</span>}
               </h2>
             </div>
          </div>
        ...
        <label className="text-[#8C7A7A] text-xs uppercase font-bold tracking-widest block mb-2">{t("auto.t_33", "На складі")}</label>
        <p className="text-[#D4AF37] text-5xl mb-8 font-black">
          {quantity} <span className="text-xl font-medium text-[#8C7A7A]">{unit}</span>
        </p>

        <label className="text-[#8C7A7A] text-[10px] uppercase font-bold tracking-widest block mb-2">{t("auto.t_34", "Собівартість за 1")}{unit}</label>
        ...
        <p className="text-[#8C7A7A] text-[10px] mt-2 px-1">
          {isMix ? t('inventoryDetail.mixPriceDesc', 'Ціна міксу розраховується автоматично на основі його складу.') : t('inventoryDetail.manualPriceDesc', 'Ви можете змінити цю ціну вручну, або вона перерахується автоматично при поповненні складу нижче.')}
        </p>
      </div>
      ...
      <div className="bg-[#1E1919] border border-[#2A2323] rounded-[32px] p-6 shadow-lg mb-6">
          ...
          {mixItems.length === 0 ?
        <p className="text-[#8C7A7A] text-xs text-center py-4 font-medium border border-dashed border-[#2A2323] rounded-2xl">{t("auto.t_36", "Кошик порожній. Додайте фрукти нижче!")}</p> :
        ...
        <div className="space-y-3">
          ...
          <div key={idx} className="flex justify-between items-center py-2 border-b border-[#2A2323] last:border-0">
                   <div>
                     ...
                     <p className="text-[#8C7A7A] text-[10px] mt-0.5">{t("auto.t_37", "Витрачено:")}{comp.cost.toFixed(2)} ₴</p>
                   </div>
      ...
      <div className="bg-[#1E1919] border border-[#2A2323] rounded-[32px] p-6 shadow-lg mb-6">
        <label className="text-[#8C7A7A] text-[10px] uppercase font-bold tracking-widest mb-3 block">{t("auto.t_38", "Надходження / Списання")}</label>
        
        {isMix &&
        <input type="text" placeholder={t("auto.t_39", "Що саме додаємо? (напр. Полуниця)")} value={compName} onChange={(e) => setCompName(e.target.value)} className="w-full bg-[#151212] border border-[#2A2323] text-[#F4EFEA] focus:border-[#D4AF37] rounded-2xl p-4 outline-none font-bold text-sm transition-colors mb-3" />
        }

        <div className="grid grid-cols-2 gap-3 mb-4">
          <input type="number" inputMode="decimal" placeholder={`${t('inventoryDetail.qty', 'Кількість')} (${unit})`} value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full bg-[#151212] border border-[#2A2323] text-[#F4EFEA] focus:border-[#D4AF37] rounded-2xl p-4 outline-none font-bold text-center text-lg transition-colors" />
          <input type="number" inputMode="decimal" placeholder={t("auto.t_40", "Вартість (₴)")} value={addedCost} onChange={(e) => setAddedCost(e.target.value)} className="w-full bg-[#151212] border border-[#2A2323] text-[#D4AF37] focus:border-[#D4AF37] rounded-2xl p-4 outline-none font-bold text-center text-lg transition-colors" />
        </div>
        ...
        <div className="flex gap-3">
          <button onClick={handleAdd} disabled={!amount || isMix && !compName || isMix && !addedCost} className="flex-1 bg-[#D4AF37] text-[#151212] shadow-lg shadow-[#D4AF37]/20 py-3.5 rounded-xl font-bold uppercase tracking-widest text-xs active:scale-95 disabled:opacity-50">{t("auto.t_41", "Поповнити")}

          </button>
          <button onClick={handleSubtract} disabled={!amount} className="flex-1 bg-[#151212] text-[#F4EFEA] border border-[#2A2323] py-3.5 rounded-xl font-bold uppercase tracking-widest text-xs active:scale-95 disabled:opacity-50">{t("auto.t_42", "Списати")}

          </button>
          <button onClick={handleWasteAction} disabled={!amount} className="flex-1 bg-red-900/20 text-red-400 border border-red-500/20 py-3.5 rounded-xl font-bold uppercase tracking-widest text-xs active:scale-95 disabled:opacity-50">{t("auto.t_43", "В брак")}

          </button>
        </div>
        <p className="text-center text-[#8C7A7A] text-[9px] mt-4 uppercase tracking-widest leading-relaxed">
           {isMix ? t('inventoryDetail.mixAddDesc', 'При поповненні введіть назву фрукта. При списанні - складники зменшаться пропорційно.') : t('inventoryDetail.itemAddDesc', 'При поповненні з вказаною вартістю, програма перерахує середню ціну. "В брак" зафіксує фінансові збитки.')}
        </p>
      </div>
      ...
      <div className="bg-[#1E1919] border border-[#2A2323] rounded-[32px] p-6 shadow-lg mb-6">
        ...
        <p className="text-[#8C7A7A] text-[10px] mb-4">{t("auto.t_45", "Введіть фактичну вагу товару, яку показують ваші ваги. Програма сама вирахує різницю.")}</p>
        <div className="flex gap-3">
          <input type="number" inputMode="decimal" placeholder={`${t('inventoryDetail.actual', 'Фактично')} (${unit})`} value={auditAmount} onChange={(e) => setAuditAmount(e.target.value)} className="w-2/3 bg-[#151212] border border-[#2A2323] text-[#F4EFEA] focus:border-[#D4AF37] rounded-2xl p-4 outline-none font-bold text-center text-lg transition-colors" />
          <button onClick={handleAudit} disabled={auditAmount === ''} className="w-1/3 bg-[#151212] text-[#D4AF37] border border-[#D4AF37]/30 py-4 rounded-2xl font-bold uppercase tracking-widest text-xs active:scale-95 disabled:opacity-50">{t("auto.t_46", "Звірити")}
      ...
      <div className="bg-[#1E1919] border border-[#2A2323] rounded-[32px] p-6 shadow-lg">
        <label className="text-[#8C7A7A] text-[10px] uppercase font-bold tracking-widest mb-4 flex items-center gap-1.5"><History size={14} />{t("auto.t_47", "Історія руху товару")}</label>
        {itemLogs.length === 0 ?
        <p className="text-[#8C7A7A] text-xs text-center py-4">{t("auto.t_48", "Історія порожня")}</p> :
