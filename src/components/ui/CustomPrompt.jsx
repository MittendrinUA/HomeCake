...
export default function CustomPrompt({ config, onClose }) {const { t } = useTranslation();
    ...
    <div className="fixed inset-0 bg-black/80 z-[200] flex items-center justify-center p-5 animate-in fade-in duration-100 backdrop-blur-sm">
      <div className="bg-[#1E1919] border border-[#2A2323] p-6 rounded-[32px] w-full max-w-sm shadow-2xl">
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={handleSubmit} className="flex-1 bg-[#D4AF37] text-[#151212] font-bold py-3.5 rounded-xl shadow-lg shadow-[#D4AF37]/20 active:scale-95">{t("auto.t_110", "Зберегти")}</button>
          <button onClick={onClose} className="flex-1 bg-[#151212] text-[#F4EFEA] border border-[#2A2323] font-bold py-3.5 rounded-xl active:scale-95">{t("auto.t_112", "Скасувати")}</button>
        </div>
      </div>
    </div>);
