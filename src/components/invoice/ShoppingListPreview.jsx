...
const ShoppingListPreview = memo(function ShoppingListPreview({ inventory, sales, recipes, preps }) {const { t } = useTranslation();
    ...
    <div className="p-4 pb-28">
      <div className="bg-[#FDFBF7] text-[#2A2323] p-6 rounded-sm shadow-lg max-w-full font-mono text-sm relative mb-6 border border-[#D4AF37]/30">
        <div className="text-center mb-6 border-b-2 border-dashed border-[#8C7A7A]/30 pb-4 flex flex-col items-center">
          ...
          <p className="text-[#8C7A7A] text-xs">{t("auto.t_62", "Список закупівель")}</p>
          <p className="font-bold mt-2">{dt}</p>
        </div>
        
        {def.length === 0 ?
        <div className="text-center py-10">
            <p className="font-bold text-lg text-[#5B7A5A]">{t("auto.t_63", "Склад повністю готовий! 📦")}</p>
            <p className="text-[#8C7A7A] mt-2">{t("auto.t_64", "Інгредієнтів вистачає на всі замовлення.")}</p>
          </div> :
      ...
      <button onClick={() => {window.open(`https://t.me/share/url?url=${encodeURIComponent(`🛒 *СПИСОК ПОКУПОК Whisked* (${dt})\n\n` + def.map((i) => `🔹 ${i.name} — ${i.missingAmt} ${i.unit}`).join('\n'))}`, '_blank');}} className="w-full bg-[#2AABEE] text-white font-bold py-4 rounded-xl shadow-xl flex items-center justify-center gap-2 active:scale-95 text-lg">
          <Send size={20} />{t("auto.t_65", "Відправити в Telegram")}
      </button>
      }
    </div>);
