    ...
    <div className="p-4 flex flex-col h-full">
      <div className="flex items-center justify-between mb-4 mt-2">
        <h2 className="text-xl font-bold text-[#F4EFEA] flex items-center gap-2">
          <Trash2 className="text-[#D4AF37]" size={24} />{t("auto.t_72", "Кошик")}

        </h2>
        {deletedItems.length > 0 &&
        <button
          onClick={onEmptyTrash}
          className="text-xs text-red-400 font-bold tracking-widest uppercase bg-red-400/10 px-3 py-1.5 rounded-lg active:scale-95 transition-transform">{t("auto.t_117", "Очистити все")}
      ...
      <div className="bg-[#D4AF37]/10 border border-[#D4AF37]/30 p-3 rounded-xl mb-4 flex gap-3 items-start">
        ...
        <p className="text-[#F4EFEA] text-sm leading-snug">{t("auto.t_118", "Елементи в кошику автоматично видаляються через")}
          <span className="font-bold text-[#D4AF37]">{t("auto.t_119", "30 днів")}</span>.
        </p>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center px-4 opacity-50 pb-20">
          ...
          <p className="text-lg font-bold text-[#F4EFEA]">{t("auto.t_120", "Кошик порожній")}</p>
          <p className="text-sm text-[#8C7A7A] mt-1">{t("auto.t_121", "Тут будуть відображатися видалені файли")}</p>
        </div> :
        ...
        <div key={item.id + item.colName} className="bg-[#1E1919] p-4 rounded-[20px] border border-[#2A2323] flex items-center justify-between gap-3 shadow-md">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[10px] text-red-400 font-bold">{t("auto.t_122", "Залишилось:")}
