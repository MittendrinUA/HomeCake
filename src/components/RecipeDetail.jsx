                ...
                <div key={ing.oIdx} className={`p-4 flex justify-between items-center ${lIdx !== 0 ? 'border-t border-[#2A2323]' : ''}`}>
                    <div>
                      <h3 className="text-[#F4EFEA] font-medium text-base flex items-center gap-1.5">
                        {isPrepL && <span className="bg-[#D4AF37]/20 text-[#D4AF37] px-1.5 py-0.5 rounded text-[8px] uppercase tracking-widest font-bold">{t("auto.t_103", "ТК")}</span>}
                        {!isPrepL && linkedItem.isMix && <span className="bg-[#2AABEE]/20 text-[#2AABEE] px-1.5 py-0.5 rounded text-[8px] uppercase tracking-widest font-bold">{t("auto.t_32", "Мікс")}</span>}
                        {linkedItem.name}
                      </h3>
    ...
    <div className="pb-28">
      ...
      <div className="mx-4 mb-6">
           <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-[#8C7A7A] text-[10px] uppercase font-bold tracking-widest block mb-2 px-2">{t('recipeDetail.priceFor') || 'Прайс (за'} {item.baseYield || 1}{getUnit(item.unit || 'шт')})</label>
      ...
      <div className="mx-4 mb-8">
        ...
        <div className="bg-[#1E1919] border border-[#2A2323] p-4 rounded-2xl mb-4">
            ...
            <div className="flex gap-2 mb-3">
              ...
              <input type="text" placeholder={t("auto.t_105", "Група")} value={newIngGroup} onChange={(e) => setNewIngGroup(e.target.value)} className="w-1/2 bg-[#151212] border border-[#2A2323] text-[#F4EFEA] focus:border-[#D4AF37] p-4 rounded-xl outline-none text-center transition-colors shadow-inner" />
            </div>
              ...
              <div className="bg-[#151212] border border-[#2A2323] p-4 rounded-2xl mb-4">
                  ...
                  className="w-full bg-[#1E1919] text-[#F4EFEA] border border-[#2A2323] p-4 rounded-xl mb-3 outline-none" />
                    ...
                    <div className="flex gap-2 mb-3">
                      ...
                      <input type="text" placeholder={t("auto.t_105", "Група")} value={newIngGroup} onChange={(e) => setNewIngGroup(e.target.value)} className="w-1/2 bg-[#1E1919] border border-[#2A2323] text-[#F4EFEA] focus:border-[#D4AF37] p-4 rounded-xl outline-none text-center transition-colors shadow-inner" />
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => {
                    if (newIngFullId && newIngAmount) {
