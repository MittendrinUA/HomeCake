...
export default function App() {
  ...
  // RENDER HELPERS
  let headerTitle = '';
  if (showInvoicePreview) headerTitle = t('menu.invoice', 'Товарний чек');else
  if (showShoppingListPreview) headerTitle = t('menu.shoppingList', 'Список закупівель');else
  if (editingOrder) headerTitle = t('common.editing', 'Редагування');else
  if (isAddingNew) headerTitle = activeTab === 'sales' ? t('orders.new', 'Нове замовлення') : activeTab === 'inventory' ? t('inventory.new', 'Новий матеріал') : activeTab === 'customers' ? t('customers.new', 'Новий клієнт') : t('common.newRecord', 'Новий запис');else
  if (selectedItem) headerTitle = selectedItem.name;else
  if (invoiceMode) headerTitle = `${t('common.selected', 'Вибрано:')} ${selectedForInvoice.length}`;else
  if (activeTab === 'sales') headerTitle = t('menu.sales', 'Замовлення');else
  if (activeTab === 'inventory') headerTitle = t('menu.inventory', 'Склад');else
  if (activeTab === 'customers') headerTitle = t('menu.customers', 'Клієнти');else
  if (activeTab === 'recipes') headerTitle = selectedCategory ? selectedCategory : t('menu.recipes', 'Каталог');else
  if (activeTab === 'preps') headerTitle = t('menu.preps', 'Заготівлі');else
  if (activeTab === 'analytics') headerTitle = t('menu.analytics', 'Аналітика');else
  if (activeTab === 'admin') headerTitle = t('menu.admin', 'Адмін Панель');else
  if (activeTab === 'trash') headerTitle = t('menu.trash', 'Кошик');
            ...
            <AnimatePresence mode="wait">
              ...
              initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.1 }} className="flex-1">
                ...
                activeTab === 'sales' ? <AddOrderForm recipes={activeRecipes} inventory={activeInventory} preps={activePreps} onSave={async (d, f) => {
                  ...
                  try {
                    if (f) {const { updateDoc } = await import('firebase/firestore');await updateDoc(doc(db, 'bakery', user.uid, 'sales', f.id), d);showToast("Оновлено!");} else
                    {await addDoc(collection(db, 'bakery', user.uid, 'sales'), { ...d, createdAt: Date.now() });showToast("Збережено!");}
                    handleBack();
                ...
                <div className="pt-4">
                    ...
                    if (id) handleUpdateDoc('categories', id, { name: newName, icon });else
                    addDoc(collection(db, 'bakery', user.uid, 'categories'), { name: newName, icon });
                  ...
                  <div className="pt-2">
                        <div className="flex bg-[#1E1919] mx-4 rounded-[20px] p-1.5 mb-2 shadow-sm border border-[#2A2323]">
                          <button onClick={() => setInventoryMode('list')} className={`flex-1 py-2.5 text-xs uppercase tracking-widest font-bold rounded-xl transition-all border ${inventoryMode === 'list' ? 'bg-[#151212] text-[#D4AF37] shadow-md border-[#2A2323]' : 'text-[#8C7A7A] border-transparent'}`}>{t("auto.t_2", "Залишки")}</button>
                          <button onClick={() => setInventoryMode('history')} className={`flex-1 py-2.5 text-xs uppercase tracking-widest font-bold rounded-xl transition-all border ${inventoryMode === 'history' ? 'bg-[#151212] text-[#F4EFEA] shadow-md border-[#2A2323]' : 'text-[#8C7A7A] border-transparent'}`}>{t("auto.t_3", "Історія руху")}</button>
                        </div>
                        {inventoryMode === 'list' ?
        ...
        <div className="absolute inset-0 bg-black/80 z-[100] flex items-center justify-center p-5 animate-in fade-in duration-100">
            <div className="bg-[#1E1919] border border-[#2A2323] p-6 rounded-[32px] w-full max-w-xs shadow-2xl">
              ...
              <div className="flex gap-3">
                <button onClick={() => {confirmDialog.onConfirm();setConfirmDialog(null);}} className="flex-1 bg-[#D4AF37] text-[#151212] font-bold py-3.5 rounded-xl shadow-lg shadow-[#D4AF37]/20 active:scale-95">{t("auto.t_5", "Так")}</button>
                <button onClick={() => {if (confirmDialog.onCancel) confirmDialog.onCancel();setConfirmDialog(null);}} className="flex-1 bg-[#151212] text-[#F4EFEA] border border-[#2A2323] font-bold py-3.5 rounded-xl active:scale-95">{t("auto.t_6", "Ні")}</button>
              </div>
            </div>
          </div>
        }
