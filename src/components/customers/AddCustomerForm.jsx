...
const AddCustomerForm = memo(function AddCustomerForm({ onSave }) {const { t } = useTranslation();
  ...
  return (
    <div className="p-4 h-full">
       <div className="bg-[#1E1919] border border-[#2A2323] rounded-[32px] p-6 shadow-lg">
          <label className="text-[#8C7A7A] text-[10px] uppercase font-bold mb-2 block tracking-widest">{t("auto.t_14", "Ім'я клієнта (обов'язково)")}</label>
          ...
          <label className="text-[#8C7A7A] text-[10px] uppercase font-bold mb-2 block tracking-widest">{t("auto.t_15", "Телефон")}</label>
          ...
          <label className="text-[#8C7A7A] text-[10px] uppercase font-bold mb-2 block tracking-widest">{t("auto.t_16", "Нотатки")}</label>
          ...
          <button onClick={() => onSave({ name, phone, instagram, notes })} disabled={!name} className="w-full bg-[#D4AF37] disabled:opacity-50 text-[#151212] py-4 rounded-2xl font-bold uppercase tracking-widest text-sm shadow-lg shadow-[#D4AF37]/20 active:scale-95">{t("auto.t_17", "Додати клієнта")}</button>
       </div>
    </div>);
