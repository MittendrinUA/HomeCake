            ...
            <AnimatePresence mode="wait">
              <motion.div key={activeTab + (selectedItem ? 'item' : '') + (isAddingNew ? 'new' : '') + (selectedCategory || '') + (showInvoicePreview ? 'inv' : '') + (showShoppingListPreview ? 'shop' : '')}
                ...
                {showInvoicePreview ? (
                  ...
                  <div className="pt-4">
                    {activeTab === 'sales' && <OrdersList sales={activeSales} recipes={activeRecipes} costFn={calculateCost} onDelete={(id) => handleDeleteDoc('sales', id, "Перемістити замовлення у Кошик?")} onComplete={handleCompleteOrder} invoiceMode={invoiceMode} selectedForInvoice={selectedForInvoice} toggleSelection={(id) => setSelectedForInvoice(prev => prev.includes(id) ? prev.filter(i=>i!==id) : [...prev, id])} onEditOrder={setEditingOrder} />}
