...
const OrderItem = memo(({ order, filter, invoiceMode, isSelected, onDelete, onComplete, onRestore, toggleSelection, onEditOrder }) => {const { t } = useTranslation();
    ...
    <motion.div
      ...
      <motion.div
          ...
          <div className="flex gap-3 items-start w-full">
            ...
            <div className="flex-1">
              ...
              <div className="bg-[#151212] rounded-2xl p-4 mb-4 border border-[#2A2323] pointer-events-auto">
                ...
                {order.decorPrice > 0 && <div className="flex justify-between py-2 text-[#8C7A7A] text-xs mt-1 border-t border-[#2A2323] pt-2 font-medium"><span>{t("auto.t_90", "+ Декор / Коробка")}</span><span className="text-[#F4EFEA]">{order.decorPrice} ₴</span></div>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 p-3 rounded-2xl border border-[#2A2323] pointer-events-none" style={{ backgroundColor: isSelected ? 'rgba(212, 175, 55, 0.05)' : '#151212' }}>
          <div className="text-center"><p className="text-[8px] uppercase font-bold text-[#8C7A7A] mb-1 tracking-widest">{t("auto.t_91", "Заг. Чек")}</p><p className="text-[#D4AF37] font-bold text-base">{order.tr.toFixed(2)}₴</p></div>
          <div className="text-center border-l border-r border-[#2A2323]"><p className="text-[8px] uppercase font-bold text-[#8C7A7A] mb-1 tracking-widest">{t("auto.t_92", "Витрати")}</p><p className="text-[#F4EFEA] font-bold text-base">{order.tc.toFixed(2)}₴</p></div>
          <div className="text-center"><p className="text-[8px] uppercase font-bold text-[#8C7A7A] mb-1 tracking-widest">{t("auto.t_93", "Прибуток")}</p><p className="text-[#5B7A5A] font-black text-base">{order.profit > 0 ? '+' : ''}{order.profit.toFixed(2)}₴</p></div>
        </div>
        
        {filter === 'planned' && !invoiceMode &&
        <p className="text-center text-[#8C7A7A] text-[10px] mt-4 flex justify-center items-center gap-1 uppercase tracking-widest font-bold pointer-events-none opacity-60"><ChevronRight size={14} />{t("auto.t_94", "Свайп вправо для видачі")}<ChevronRight size={14} /></p>
        }
      </motion.div>
    </motion.div>);
...
const OrdersList = memo(function OrdersList({ sales, recipes, costFn, onDelete, onComplete, onRestore, invoiceMode, selectedForInvoice, toggleSelection, onEditOrder }) {const { t } = useTranslation();
  ...
  return (
    <div className="pb-36">
      ...
      <div className="flex bg-[#1E1919] mx-4 rounded-[20px] p-1.5 mb-5 shadow-sm border border-[#2A2323]">
          <button onClick={() => setFilter('planned')} className={`flex-1 py-2.5 text-xs uppercase tracking-widest font-bold rounded-xl transition-all border ${filter === 'planned' ? 'bg-[#151212] text-[#D4AF37] shadow-md border-[#2A2323]' : 'text-[#8C7A7A] border-transparent'}`}>{t("auto.t_95", "Активні")}</button>
          <button onClick={() => setFilter('completed')} className={`flex-1 py-2.5 text-xs uppercase tracking-widest font-bold rounded-xl transition-all border ${filter === 'completed' ? 'bg-[#151212] text-[#F4EFEA] shadow-md border-[#2A2323]' : 'text-[#8C7A7A] border-transparent'}`}>{t("auto.t_96", "Історія")}</button>
        </div>
      }
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-8 text-center flex flex-col items-center mt-10">
              ...
              <p className="text-[#8C7A7A] font-medium">{t("auto.t_97", "Список порожній.")}</p>
            </motion.div>
          }
